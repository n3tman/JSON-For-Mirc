...
alias JSON {
  ...
  elseif (%Prop == httpHeader) {
    ...
    elseif ($jfm_Exec(%Com, httpHeader, $evalnext($ $+ %Offset))) {
      %Error = $v1
    }
    else {
      %Result = $hget(SReject/JSONForMirc, Exec)
  ...
  elseif (%Prop == $null) || ($wildtok(path|pathLength|type|isContainer|length|value|string|debug, %Prop, 1, 124)) {
    ...
    if ($0 >= %Offset) {
      ...
      if (!$evalnext(%Call)) || ($comerr) || (!$com(%ChildCom)) {
        %Error = $jfm_GetError
        goto error
      }
...
alias JSONForEach {
  ...
  elseif (!$1) || ($1 == 0) || (!$regex($1, /^((?:[^?:*]+)|(?:JSON:[^?:*]+(?::\d+)))$/)) {
    ...
    if (!%JSON) {
      ...
      if (!$evalnext(%Call)) || ($comerr) || (!$com(%Com)) {
        %Error = $jfm_GetError
      }
...
alias -l jfm_ComInit {
  ...
  if (%Error) {
    ...
    hadd -m SReject/JSONForMirc bvar 0
    hadd -m SReject/JSONForMirc com 0
...
alias -l jfm_Exec {
  ...
  while (%Index < $0) {
    ...
    if (%Index >= 3) {
      if ($prop == fromBvar) && ($regex($evalnext($ $+ %Index), /^& (&\S+)$/)) {
